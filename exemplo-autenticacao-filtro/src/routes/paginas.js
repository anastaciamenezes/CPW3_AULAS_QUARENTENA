...
//rotas para cadastrar um novo usuario
router.get('/cadastro', loginCtrl.consultaAutenticacao, (req, res) =>
    res.render('cadastro', { usuario: req.session.usuario }));

router.post('/cadastro', (req, res) => {

    const { email, nome, senha } = req.body;
    const resposta1 = loginCtrl.registrarUsuario(email, nome, senha);
    res.render('cadastro', { mensagem: resposta1.mensagem, usuario: req.session.usuario });
});

//rota para botão Logout
router.get('/botaoLogout', loginCtrl.logout);
